...
import { AIDietTimetable } from './AIDietTimetable';
import { AIExerciseTimetable } from './AIExerciseTimetable';
import { 
  Sparkles, 
  Send, 
  ShieldAlert, 
...
export const AIHealthAssistant: React.FC = () => {
  ...
  const [activeSubTab, setActiveSubTab] = useState<'chat' | 'diet' | 'exercise'>('chat');
  ...
  }, [aiMessages, isSending, activeSubTab]);
  ...
  return (
    <div className="space-y-6 animate-fadeIn">
      ...
      {/* Sub-Navigation Tabs: Chat, Diet Timetable, Exercise Timetable */}
      <div className="flex items-center gap-2 border-b border-white/[0.08] pb-1 overflow-x-auto text-xs">
        <button
          onClick={() => setActiveSubTab('chat')}
          className={clsx(
            "flex items-center gap-2 px-4 py-2.5 rounded-2xl font-bold transition-all whitespace-nowrap border",
            activeSubTab === 'chat'
        ...
        >
          ...
          <span>AI Clinical Consultation & Chat</span>
        </button>
        ...
        <button
          ...
          className={clsx(
            "flex items-center gap-2 px-4 py-2.5 rounded-2xl font-bold transition-all whitespace-nowrap border",
            activeSubTab === 'diet'
              ? "bg-gradient-to-r from-brand-teal/20 to-cyan-500/20 text-brand-cyan border-brand-teal/60 shadow-glow-teal"
              : "bg-white/[0.02] text-slate-400 border-white/[0.06] hover:bg-white/[0.05] hover:text-white"
          )}
        >
          <Utensils className="w-4 h-4 text-brand-amber" />
          <span>Weekly Diet Plan Timetable</span>
          <span className="text-[10px] font-mono px-1.5 py-0.5 rounded bg-brand-amber/20 text-brand-amber font-bold">MON-SUN</span>
        </button>
        ...
        <button
          ...
          className={clsx(
            ...
            activeSubTab === 'exercise'
              ? "bg-gradient-to-r from-brand-emerald/20 to-teal-500/20 text-brand-emerald border-brand-emerald/60 shadow-glow-emerald"
              : "bg-white/[0.02] text-slate-400 border-white/[0.06] hover:bg-white/[0.05] hover:text-white"
          )}
        >
          <Dumbbell className="w-4 h-4 text-brand-emerald" />
          <span>Safe Exercise Routine Timetable</span>
          <span className="text-[10px] font-mono px-1.5 py-0.5 rounded bg-brand-emerald/20 text-brand-emerald font-bold">100% SAFE</span>
        </button>
      </div>

      {/* SUB-VIEW 1: DIET PLAN TIMETABLE */}
      {activeSubTab === 'diet' && <AIDietTimetable />}

      {/* SUB-VIEW 2: SAFE EXERCISE TIMETABLE */}
      {activeSubTab === 'exercise' && <AIExerciseTimetable />}

      {/* SUB-VIEW 3: CLINICAL CHAT */}
      {activeSubTab === 'chat' && (
        <div className="space-y-6 animate-fadeIn">
                      ...
                      {msg.groundedSources && msg.groundedSources.length > 0 && (
                        <div className="pt-3 border-t border-white/[0.08] space-y-2">
                          ...
                          <div className="flex flex-wrap gap-2">
                            {msg.groundedSources.map((src: any, i: number) => (
                              <button
                                key={i}
                                onClick={() => {
